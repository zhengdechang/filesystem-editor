  ...
  release = async (releaseType: ReleaseType): Promise<boolean> => {
    ...
    switch (releaseType) {
      ...
      case ReleaseType.website: {
        ...
        const { storyfileName, templateZipUrl } = getStandaloneInterpreterMeta(
        ...
        if (this.manager.processReleaseFile) {
          const releaseFileMeta = await this.manager.processReleaseFile(
            storyfileName,
            storyfileData
          );
        ...
        this.manager
          ...
          .forEach((file) =>
            zip.file(
              ...
              new Blob([
                readFile(materialsStore.getFilesystemPath(file), file.isBinary),
              ])
            )
          );
