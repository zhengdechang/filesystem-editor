...
class MaterialsStore {
  ...
  addMaterialsFile = (
    ...
    const saveAsBinary =
      typeof isBinary === "undefined" ? this.isBinaryType(type) : isBinary;
  ...
  public getFileTree = (parent?: MaterialsFile): MaterialsFile[] => {
    ...
    children
      .filter((file: MaterialsFile) => file.type === MaterialsFileType.folder)
      .forEach(
        (file: MaterialsFile) => (file.children = this.getFileTree(file))
      );
  ...
  public moveCompilationOrder = (
    ...
    const parentId =
      typeof target.parent === "string" ? target.parent : target.parent?.id;
    ...
    reader.onload = action((): void => {
      ...
      if (existingFile) {
        ...
        this.addMaterialsFile(
          binary
            ? Buffer.from(this.binaryStringToUint8Array(binaryStr as string))
            : binaryStr,
          {
            name: file.name,
            parent,
  ...
  public serialize = (): MaterialsFile[] => {
    return this.files.map((file) => ({
      ...
      parent:
        file.parent &&
        (typeof file.parent === "string" ? file.parent : file.parent.id),
      children: undefined,
    }));
  };
